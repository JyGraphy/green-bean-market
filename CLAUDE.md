...
7. **배포 후 자동 헬스체크 (재발 방지의 핵심)** — `scripts/healthcheck.py`가 매일 갱신·배포
   이후(`sitecheck.yml`, KST 07:15 스케줄) **실제 라이브 사이트**를 점검한다: 사이트 루트 200,
   상품 수 임계치, **상대경로/우리도메인 가리키는 url 0건**, store 소멸 여부. 실패 시 잡이 빨간
   X → 소유자에게 알림 메일 → 사용자가 발견하기 전에 잡는다. 커밋 차단(commit-time)이 못 잡는
   배포·서빙 단계 문제(캐시·인증게이트 등)를 잡는 최후 그물.
8. **클라이언트 캐시 무력화** — 프론트는 `data/*.json`을 `?v=날짜` + `cache:'no-cache'`로
   받고, `vercel.json`이 `/data/*`에 `max-age=0, must-revalidate`를 준다. 데이터가 갱신돼도
   브라우저가 옛 데이터(상대경로 등)를 계속 쓰는 일을 막는다.
9. **워크플로 동작** — 개별 스크래퍼 실패는 격리(`continue-on-error`)되어 정상 store는 커밋되고,
