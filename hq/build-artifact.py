...
# 새 보고서가 생기면 여기에 추가한다.
REPORT_SOURCES = [
    ('vault/wiki/index.md', 'system', 'qa',
     '세컨드 브레인 목차 — 정리된 주제와 원본 문서 전체'),
    ('vault/wiki/*.md', 'system', 'qa',
     '정리된 지식 (wiki) — 주제별 누적 문서'),
    ('vault/raw/auctions/*.md', 'auction-reporter', 'res',
     'COE·BOP 등 옥션 전체 랏 결과 — 순위·농장·품종·낙찰가·낙찰자'),
    ('vault/raw/coe/*.md', 'auction-reporter', 'res',
     'COE 옥션 (구 경로)'),
    ('vault/raw/papers/번역/*.md', 'coffee-research-translator', 'res',
     '논문 전문 한글 완역'),
    ('vault/raw/papers/fulltext/*.md', 'coffee-research-translator', 'res',
     '논문 원문 (영어) — GitHub Actions 자동 수집'),
    ('vault/raw/roast-profiles/machines/*.md', 'roast-profile-collector', 'res',
     '로스터기별 판독 지식 — AI 프롬프트 주입 원본'),
    ('vault/raw/stores/*.md', 'store-scout', 'ops',
     '미등록 생두 쇼핑몰 후보 조사 (승인 대기)'),
    ('vault/raw/qa/*-스크래퍼점검.md', 'scraper-checker', 'qa',
     '스크래퍼 안전장치 규칙 점검 — qa_report.py 자동 생성'),
    ('vault/raw/qa/*-데이터검증.md', 'data-validator', 'qa',
     '상품 데이터 검증 — qa_report.py 자동 생성'),
    ('vault/raw/papers/*.md', 'coffee-research-translator', 'res',
     '커피 과학 논문 한글 정리'),
    ('vault/raw/roast-profiles/*.md', 'roast-profile-collector', 'res',
     '로스팅 프로파일 출처 카탈로그'),
]
